...
couples_list=np.array([ # names_list
...
def data_gen(wich_model=utils.MOD_model,pars=np.array([5e-16]),clocks_pars=utils.clocks_pars_0,Ts=utils.Ts,nyr=3.01,dt=1):
...
def chain_gen(wich_model=utils.MOD_model,pars=np.array([5e-16]),clocks_pars=utils.clocks_pars_0,Ts=utils.Ts,nyr=3.01,dt=1,log_prior=lambda _:0.0):
...
def Fisher(wich_dmodel=[utils.MOD_model_d],pars = np.array([5e-16]),clocks_pars=utils.clocks_pars_0,Ts=utils.Ts,nyr=3.01,dt=1):
...
def plotter_posterior(wich_model=utils.MOD_model,pars=np.array([5e-16]),
            ...
            clocks_pars=utils.clocks_pars_0,Ts=utils.Ts,nyr=3.01,dt=1,log_prior=lambda _:0.0, labels=['A']):
