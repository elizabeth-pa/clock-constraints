...
clocks_pars_0=[constants.h0a,constants.hm1a,constants.Ka,constants.h0b,constants.hm1b,constants.Kb]
...
def noise_generator(clocks_pars=clocks_pars_0,Ts= constants.Ts,nyr=3.01,dt=1):
...
def MOD_model(f, pars = np.array([5e-16]),Ts=constants.Ts,nyr=3.01,dt=1):
    A = pars[0]
    omega = 2*np.pi/Ts
    phi = 0.
    T=Ts*nyr
...
def MOD_model_d(f, pars = np.array([5e-16]),Ts=constants.Ts,nyr=3.01,dt=1):
    omega = 2*np.pi/Ts
    phi = 0.
    T=Ts*nyr
...
def DE_model(f,pars=np.array([1e-33]),Ts=constants.Ts,nyr=3.01,dt=1):
    T=Ts*nyr
    a=pars[0]
    return -(1/dt)*a*np.exp(-2j*np.pi*f*T)*(np.exp(2j*np.pi*f*T)-1-2j*np.pi*f*T)/(2*np.pi*f)**2

def DE_model_d(f,pars=np.array([1e-33]),Ts=constants.Ts,nyr=3.01,dt=1):
    T=Ts*nyr
...
def DM_model(f,pars=np.array([2e-21,2*np.pi/(constants.Ts),1.]),Ts=constants.Ts,nyr=3.01,dt=1):
    A=pars[0]
    om=pars[1]
    phi=pars[2]
    T=Ts*nyr
...
def DM_model_dA(f, pars = np.array([2e-21,2*np.pi/constants.Ts,1.]),Ts=constants.Ts,nyr=3.01,dt=1):
    A = pars[0]
    omega = pars[1]
    phi = pars[2]
    T=Ts*nyr
...
def DM_model_dom(f, pars = np.array([2e-21,2*np.pi/constants.Ts,1.]),Ts=constants.Ts,nyr=3.01,dt=1):
    A = pars[0]
    omega = pars[1]
    phi = pars[2]
    T=Ts*nyr
...
def DM_model_dphi(f, pars = np.array([2e-21,2*np.pi/constants.Ts,1.]),Ts=constants.Ts,nyr=3.01,dt=1):
    A = pars[0]
    omega = pars[1]
    phi = pars[2]
    T=Ts*nyr
